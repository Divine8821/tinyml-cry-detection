...
def run_ma_test():
    ...
    noise_rec = wfdb.rdrecord(os.path.join(NST_DIR, 'ma'), channels=[0])
    ...
    for snr in snr_levels:
        print(f"Testing Muscle Artifacts at {snr}dB...")
    ...
    pd.DataFrame(results).to_csv('results/ma_results.csv', index=False)
    print("Muscle Artifact test complete. Results saved to results/ma_results.csv")
