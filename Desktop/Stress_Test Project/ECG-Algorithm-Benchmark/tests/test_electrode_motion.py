...
def run_em_test():
    ...
    noise_rec = wfdb.rdrecord(os.path.join(NST_DIR, 'em'), channels=[0])
    ...
    for snr in snr_levels:
        print(f"Testing Electrode Motion at {snr}dB...")
    ...
    pd.DataFrame(results).to_csv('results/em_results.csv', index=False)
    print("Electrode motion test complete. Results saved to results/em_results.csv")
